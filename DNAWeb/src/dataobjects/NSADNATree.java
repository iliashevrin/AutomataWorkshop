...
public class NSADNATree {
	...
	/*
	 * The index of the tree in the DNA. Used by NBA.convertToDNA
	 */
	...
	/*
	 * Generates Q0 of the DNA related to the given NBA. With non-fully initialized array representation.
	 */
	public NSADNATree(NSA nsa)
	{
		this.nsa = nsa;
	...
	{
		NSADNATree ret = new NSADNATree(this.nsa); //Generate a new tree
	...
	private void fixSeniority(int[] availableAnnotations)
	{
		...
		for(int state = 0; state < statesMap.length; state++)
		{
			...
			while(node != null)
			{
				...
				if(child == null)
				{
					if(node.states.contains(state)) //TODO more elegant way?
	...
	private int fixUnique(int[] availableAnnotations)
	{
		...
		for(int i = 1; i < tree.length; i++)
		{
			if(tree[i] < tree.length)
			{
				...
				else if(!hasUnique[tree[i]] && !hasTemplarChild[tree[i]] && annotations[tree[i]] >= 0)//TODO 0 or -1?
				{
		...
		//TODO fix what is returned if k == Integer.MAX_VALUE
	...
	{
		public NSADNATree originalState;
		public NSADNATree resultState;
		public int k;
		
		public DNATransition(NSADNATree originalState, NSADNATree resultState, int k) {
